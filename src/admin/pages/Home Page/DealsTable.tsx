...
const DealsTable = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<DailyDiscount | null>(null);
  ...
  const resolveLive = (item: DailyDiscount) => {
  ...
  const toggleStatus = async (item: DailyDiscount) => {
    if (!item.id) return;
    ...
    <>
        ...
        <Table sx={{ minWidth: 980 }}>
          <TableHead>
            <TableRow sx={{ backgroundColor: "#1E293B" }}>
              {["Preview", "Title", "Discount", "Window", "Flags", "Status", "Actions"].map((head) => (
                <TableCell
                  key={head}
                  sx={{
                    color: "#fff",
              ...
              allDiscounts.map((item) => (
                <TableRow key={item.id} hover>
                  ...
                  <TableCell>
                    ...
                    <Typography sx={{ fontSize: 11, color: "#64748B", mt: 0.5 }}>{item.subtitle || "-"}</Typography>
                  </TableCell>
                  <TableCell>
                    <Typography sx={{ fontSize: 13, fontWeight: 800, color: "#0F766E" }}>
                      {item.discountLabel || `${item.discountPercent}% OFF`}
                    </Typography>
                    <Typography sx={{ fontSize: 11, color: "#64748B" }}>{item.discountPercent}%</Typography>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Box sx={{ display: "flex", gap: 0.8, flexWrap: "wrap" }}>
                      {item.highlighted && <Chip size="small" label="Highlighted" color="warning" />}
      ...
      {editTarget !== null && (
        <Modal open onClose={() => setEditTarget(null)}>
          ...
          >
            <Typography sx={{ fontSize: 18, fontWeight: 800, mb: 1.2 }}>Update Daily Discount</Typography>
            <UpdateDealForm
              id={editTarget}
              onSuccess={() => {
                setEditTarget(null);
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Delete <strong>{deleteTarget?.title || "this daily discount"}</strong>? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>

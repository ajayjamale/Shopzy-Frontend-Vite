...
const AdminSettlementDetail: React.FC = () => {
  const { id } = useParams();
  ...
  return (
    <div style={{ padding: 16, background: C.bg, minHeight: "100vh" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: C.text }}>Settlement #{id}</h1>
      ...
      {!data ? (
        ...
        >
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, marginTop: 4 }}>Seller #{data.sellerId}</div>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
              {["PENDING", "PROCESSING", "ELIGIBLE", "ON_HOLD", "COMPLETED", "FAILED", "CANCELLED"].map((s) => (
                <button
                  key={s}

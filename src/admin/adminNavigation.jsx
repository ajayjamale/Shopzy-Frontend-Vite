    ...
    items: [
      {
        ...
        description:
          'See live marketplace health, activity queues, returns, and settlements in one place.',
      },
    ],
  },
  {
...
export const isAdminNavItemActive = (location, item) => {
  const { pathname, query } = splitNavPath(item.path)
  const currentQuery = new URLSearchParams(location.search)
  const queryMatches = Array.from(query.entries()).every(
  ...
  return (
    (location.pathname === pathname || location.pathname.startsWith(`${pathname}/`)) && queryMatches
  )
}
export const findAdminNavItem = (location) =>
  adminMenuGroups
    .flatMap((group) => group.items.map((item) => ({ item, group })))
    .find(({ item }) => isAdminNavItemActive(location, item))
export const getAdminPageMeta = (location) => {
...
export const getAdminContextLinks = (location) => {
  const match = findAdminNavItem(location)
  if (match) {
...
export const getAdminBreadcrumbs = (location) => {

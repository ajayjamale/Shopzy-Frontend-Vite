    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      {products.products.slice(0, 10).map((item: any) => (
        ...
        >
          ...
          <div className="p-3">
            <p className="text-xs text-slate-500 mb-1">{item.seller?.businessDetails?.businessName || "Shopzy"}</p>
            <p className="text-sm font-semibold text-slate-800 line-clamp-2">{item.title}</p>
            <p className="text-sm font-bold text-slate-900 mt-2">Rs. {item.sellingPrice?.toLocaleString("en-IN")}</p>
          </div>
        </button>
      ))}
    </div>

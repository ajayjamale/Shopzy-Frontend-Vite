  ...
  return (
    <article className="product-card" onClick={goToDetails}>
      <div className="product-card-media">
        ...
        {item.discountPercent > 0 && <span className="product-card-badge">{item.discountPercent}% OFF</span>}
      ...
      <div className="product-card-body">
        <p className="product-card-seller">{item.seller?.businessDetails?.businessName || "Shopzy Seller"}</p>
        ...
        <div className="product-card-price">
          ...
          {item.mrpPrice > item.sellingPrice && (
            <>
              ...
              <span className="off">Save Rs. {(item.mrpPrice - item.sellingPrice).toLocaleString("en-IN")}</span>
            </>
          )}
        </div>

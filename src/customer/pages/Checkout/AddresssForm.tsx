...
interface AddressFormProps {
  ...
  onAddressSaved?: (address: Address) => Promise<void> | void;
}

const schema = Yup.object().shape({
  name: Yup.string().min(2, "Name is too short").required("Required"),
  mobile: Yup.string().matches(/^[6-9]\d{9}$/, "Use a valid 10-digit mobile").required("Required"),
  pinCode: Yup.string().matches(/^\d{6}$/, "Use a valid 6-digit PIN").required("Required"),
...
const AddressForm = ({ handleClose, onAddressSaved }: AddressFormProps) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      setSaving(true);
      try {
        await onAddressSaved?.(values as Address);
      } finally {
        setSaving(false);
      }
    },
    ...
    <div className="surface p-5" style={{ width: "min(560px, 94vw)", borderRadius: 18 }}>
      ...
      <form onSubmit={formik.handleSubmit} className="grid gap-3">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button type="submit" className="btn-primary" disabled={!formik.isValid || formik.isSubmitting || saving}>

...
const fallback = [
  { target: "men", name: "Men", image: "https://rukminim2.flixcart.com/image/832/832/xif0q/t-shirt/e/i/b/4xl-bys-hd-fs7-btny-fubar-original-imahfgfetkgbtuhr.jpeg?q=70&crop=false" },
  { target: "women", name: "Women", image: "https://rukminim2.flixcart.com/image/832/832/xif0q/dress/j/a/g/l-ss-11-sheetal-associates-original-imahf7qyzjjjzrez.jpeg?q=70&crop=false" },
  { target: "electronics", name: "Electronics", image: "https://rukminim2.flixcart.com/image/312/312/xif0q/computer/x/9/j/-original-imahyjzh7m2zsqdg.jpeg?q=70" },
  { target: "home_furniture", name: "Home & Furniture", image: "https://images.unsplash.com/photo-1493666438817-866a91353ca9?w=600&q=80&fit=crop" },
];
...
const HomeCategory = () => {
  ...
  const categories =
    homePage.homePageData?.shopByCategories?.map((item: any) => ({
      target: item.redirectLink || item.categoryId || "men",
      name: item.name || item.title || "Collection",
      image: item.image || item.imageUrl,
    })) || fallback;

  const featuredDeal = useMemo<DealPreview>(() => {
    const liveDeals = ((homePage.homePageData?.deals || []) as DealPreview[]).filter(
  ...
  const dealTitle = (
    featuredDeal.title ||
    featuredDeal.category?.categoryId ||
    "today's picks"
  )
    .split("_")
    .join(" ");
          ...
          {categories.slice(0, 8).map((item) => (
            ...
            >
              ...
              <div className="p-4 flex items-center justify-between">
                ...
                <span className="text-xs font-bold tracking-[0.1em] uppercase text-teal-700">Explore</span>
              </div>
            </button>
          ))}
        </div>

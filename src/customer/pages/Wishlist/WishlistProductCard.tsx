  ...
  return (
    ...
    >
      ...
      <div className="p-3 grid gap-1">
        <p className="text-xs text-slate-500">{item.seller?.businessDetails?.businessName || "Shopzy Seller"}</p>
        <p className="text-sm font-semibold text-slate-800 line-clamp-2">{item.title}</p>
        <p className="text-sm font-bold text-slate-900 mt-1">Rs. {item.sellingPrice?.toLocaleString("en-IN")}</p>
      </div>

...
type SellerTone = "default" | "accent" | "info" | "success" | "warning" | "danger";

const toneMap: Record<SellerTone, { text: string; border: string; bg: string }> = {
  default: {
...
export const humanizeSellerValue = (value?: string | null) => {
...
export const formatSellerDate = (value?: string, options?: Intl.DateTimeFormatOptions) => {
  if (!value) return "N/A";
...
export const formatSellerDateTime = (value?: string) => {
  if (!value) return "N/A";
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;
  ...
  >
    <Stack
      ...
      sx={{
        ...
        py: 2,
        borderBottom: `1px solid ${sellerPalette.border}`,
        background: `linear-gradient(180deg, ${alpha(sellerPalette.info, 0.05)} 0%, ${alpha(
          sellerPalette.accent,
          0.02
        )} 100%)`,
      }}
    >

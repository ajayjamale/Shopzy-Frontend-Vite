...
import { FormControl, MenuItem, Select, Box, Button, Stack, Typography } from '@mui/material'
...
const HomePage = () => {
  ...
  const cancellationRate =
    totalOrders > 0 ? Math.round((Number(report?.canceledOrders ?? 0) / totalOrders) * 100) : 0
  const refundRate =
    totalOrders > 0 ? Math.round((Number(report?.totalRefunds ?? 0) / totalOrders) * 100) : 0
  return (
    <Box>
      <SellerPageIntro
        eyebrow="Overview"
      ...
      >
        ...
        <SellerSection
          ...
          description="Quick reads for the next actions worth taking today."
        >
          <Stack spacing={1.6}>
            <SnapshotRow
            ...
            <SnapshotRow
              ...
              value={formatSellerCurrency(
                totalOrders > 0 ? Number(report?.totalEarnings ?? 0) / totalOrders : 0,
              )}
...
const SnapshotRow = ({ icon, label, value, detail }) => (
  <Stack
    direction="row"
    spacing={1.4}
  ...
  >
    ...
    <Box sx={{ minWidth: 0 }}>
      ...
      <Typography sx={{ color: '#0F172A', fontSize: '1.05rem', fontWeight: 900, mt: 0.4 }}>
        {value}
      </Typography>

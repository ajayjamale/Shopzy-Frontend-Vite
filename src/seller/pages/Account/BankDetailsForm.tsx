...
const BankDetailsForm = ({ onClose }: UpdateDetailsFormProps) => {
  ...
  const formik = useFormik({
    initialValues: { accountHolderName: "", accountNumber: "", ifscCode: "" },
    ...
    onSubmit: (values) => {
      dispatch(updateSeller({ bankDetails: values }));
      onClose();
    },
  });
  ...
  return (
    <form onSubmit={formik.handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <Field id="accountHolderName" name="accountHolderName" label="Account Holder Name"
      ...
      <Field id="accountNumber" name="accountNumber" label="Account Number"
        value={formik.values.accountNumber} onChange={formik.handleChange} onBlur={formik.handleBlur}
        error={formik.touched.accountNumber && Boolean(formik.errors.accountNumber)}
        helperText={formik.touched.accountNumber && formik.errors.accountNumber} />
      <Field id="ifscCode" name="ifscCode" label="IFSC Code"
        value={formik.values.ifscCode} onChange={formik.handleChange} onBlur={formik.handleBlur}

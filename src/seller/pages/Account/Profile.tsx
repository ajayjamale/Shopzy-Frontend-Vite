...
const Profile = () => {
    ...
    () => [
      {
        ...
        tone: sellers.profile?.accountStatus === "ACTIVE" ? ("success" as const) : ("warning" as const),
  ...
  return (
    <Box>
      ...
      >
        <Alert onClose={() => setToastOpen(false)} severity={sellers.error ? "error" : "success"} variant="filled">
...
const renderDrawerForm = (drawerOpen: DrawerId, onClose: () => void) => {
  switch (drawerOpen) {
    case "personal":
...
const getTabSection = (tab: TabId, profile: any) => {
  const sections = {
    personal: {

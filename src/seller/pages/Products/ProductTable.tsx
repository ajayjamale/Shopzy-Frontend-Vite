...
type ProductTableProps = {
  variant?: "products" | "inventory";
};

const ProductTable = ({ variant = "products" }: ProductTableProps) => {
  ...
  const summary = useMemo(() => {
    ...
    const catalogValue = products.reduce((sum, item) => sum + Number(item.sellingPrice || 0) * Number(item.quantity || 0), 0);
    const averageDiscount = products.length
      ? Math.round(products.reduce((sum, item) => sum + Number(item.discountPercent || 0), 0) / products.length)
      : 0;
  ...
  return (
    <Box>
      <SellerPageIntro
        ...
        actions={
          <>
            <Button variant="outlined" startIcon={<RefreshRoundedIcon />} onClick={() => dispatch(fetchSellerProducts(getSellerToken()))} sx={sellerSecondaryButtonSx}>
              Refresh
            </Button>
                    ...
                    <TableRow key={item.id} hover>
                      <TableCell sx={sellerTableCellSx}>
                        <Stack direction="row" spacing={1.4} alignItems="center">
                          <Avatar
                            ...
                            src={item.images?.[0]}
                            alt={item.title}
                            sx={{ width: 54, height: 54, borderRadius: "12px", border: "1px solid #DCE8EC" }}
                          />
                          <Box sx={{ minWidth: 0 }}>
                            <Typography sx={{ fontWeight: 800, color: "#0F172A" }}>{item.title}</Typography>
                            <Typography sx={{ fontSize: ".8rem", color: "#64748B", mt: 0.3 }}>
                              SKU #{String(item.id).padStart(6, "0")} • {humanizeSellerValue(item.color)}
                            </Typography>
                          </Box>
                        </Stack>
                      </TableCell>
                      <TableCell sx={sellerTableCellSx}>
                        <Typography sx={{ fontWeight: 700 }}>{item.category?.name || "Unassigned"}</Typography>
                        <Typography sx={{ fontSize: ".8rem", color: "#64748B", mt: 0.4 }}>
                          Size {item.sizes || "N/A"}
                        </Typography>
                      </TableCell>
                      <TableCell sx={sellerTableCellSx} align="right">
                        <Typography sx={{ fontWeight: 800 }}>{formatSellerCurrency(item.sellingPrice)}</Typography>
                        <Typography sx={{ fontSize: ".8rem", color: "#64748B", mt: 0.4 }}>
                          MRP {formatSellerCurrency(item.mrpPrice)}
                        </Typography>
                      </TableCell>
                      <TableCell sx={sellerTableCellSx} align="right">
                        <Typography sx={{ fontWeight: 800 }}>{Number(item.quantity ?? 0).toLocaleString("en-IN")}</Typography>
                        <Typography sx={{ fontSize: ".8rem", color: item.quantity <= 5 ? "#D97706" : "#64748B", mt: 0.4 }}>
                          {item.quantity <= 5 ? "Low stock" : "Healthy stock"}
                        </Typography>
                      </TableCell>
                      <TableCell sx={sellerTableCellSx} align="right">
                        <Typography sx={{ fontWeight: 800 }}>{Number(item.discountPercent || 0)}%</Typography>
                      </TableCell>
                      <TableCell sx={sellerTableCellSx}>
                        <SellerStatusChip
                      ...
                      <TableCell sx={sellerTableCellSx} align="right">
                        <Stack direction="row" spacing={1} justifyContent="flex-end" useFlexGap flexWrap="wrap">
                          <Button variant="outlined" size="small" sx={sellerSecondaryButtonSx} onClick={() => navigate(`/seller/update-product/${item.id}`)}>
                            Edit
                          </Button>
                          <Button
                            variant="outlined"

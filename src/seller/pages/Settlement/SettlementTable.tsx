...
const toneForStatus = (status: SettlementStatus) => {
  ...
  if (!items.length && !loading) {
    return (
      <SellerEmptyState
        ...
        description={error || "Settlement records will appear here as orders become eligible for payout."}
        action={
          onRetry ? (
            ...
            {items.map((item) => (
              <TableRow key={item.id} hover>
                ...
                <TableCell sx={sellerTableCellSx} align="right">
                  <Typography sx={{ fontWeight: 800 }}>{formatSellerCurrency(item.grossAmount)}</Typography>
                </TableCell>
                <TableCell sx={sellerTableCellSx} align="right">
                ...
                <TableCell sx={sellerTableCellSx}>
                  <SellerStatusChip label={humanizeSellerValue(item.settlementStatus)} tone={toneForStatus(item.settlementStatus)} small />
                </TableCell>
                <TableCell sx={sellerTableCellSx} align="right">
                  <Typography sx={{ fontWeight: 700 }}>{formatSellerDate(item.settlementDate || item.createdAt)}</Typography>
                  <Button
                    variant="text"
                    size="small"

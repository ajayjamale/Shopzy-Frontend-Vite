...
const returnManagedStatuses = new Set<OrderStatus>([
...
const statusTone = (status: string) => {
  ...
  if (status === "SHIPPED" || status === "CONFIRMED" || status === "PLACED") return "info" as const;
  if (status === "PENDING") return "warning" as const;
  if (status === "RETURN_REQUESTED" || status === "REFUND_INITIATED" || status === "RETURNED") return "accent" as const;
...
const OrderTable = () => {
  ...
  const pending = orders.filter((order) => order.orderStatus === "PENDING" || order.orderStatus === "PLACED").length;
  ...
  return (
    <Box>
      ...
      >
        <SellerMetricCard label="Total orders" value={orders.length.toLocaleString("en-IN")} helper="All seller-side order records" tone="accent" icon={<ShoppingBagRoundedIcon />} />
        <SellerMetricCard label="Pending queue" value={pending.toLocaleString("en-IN")} helper="Orders waiting for confirmation or dispatch" tone="warning" icon={<LocalShippingRoundedIcon />} />
        <SellerMetricCard label="Shipped" value={shipped.toLocaleString("en-IN")} helper="Orders in transit right now" tone="info" icon={<LocalShippingRoundedIcon />} />
        <SellerMetricCard label="Delivered / returns" value={`${delivered} / ${returnQueue}`} helper="Delivered orders and return-managed orders" tone="success" icon={<ReplayRoundedIcon />} />
      </Box>
      ...
      <Box sx={{ mt: 2 }}>
        ...
        >
          ...
          <Stack direction={{ xs: "column", lg: "row" }} spacing={1.5} sx={{ px: 2.4, py: 2, borderBottom: "1px solid #DCE8EC" }}>
            ...
            />
            <TextField select size="small" value={filter} onChange={(event) => setFilter(event.target.value)} sx={{ ...sellerInputSx, minWidth: 200 }}>
              <MenuItem value="ALL">All statuses</MenuItem>
              {[...statusOptions, "RETURN_REQUESTED", "REFUND_INITIATED", "RETURNED"].map((status) => (
    ...
    <TableRow hover>
      ...
      <TableCell sx={sellerTableCellSx}>
        <Stack direction="row" spacing={1.4} alignItems="center">
          ...
          />
          <Box sx={{ minWidth: 0 }}>
            <Typography sx={{ fontWeight: 800 }}>{primaryItem?.product.title || "Order item"}</Typography>
            <Typography sx={{ fontSize: ".8rem", color: "#64748B", mt: 0.4 }}>
      ...
      <TableCell sx={sellerTableCellSx}>
        <Typography sx={{ fontWeight: 800 }}>{order.shippingAddress?.name || "Customer"}</Typography>
        <Typography sx={{ fontSize: ".8rem", color: "#64748B", mt: 0.4 }}>
      ...
      <TableCell sx={sellerTableCellSx} align="right">
        <Typography sx={{ fontWeight: 800 }}>{formatSellerCurrency(order.totalSellingPrice || primaryItem?.sellingPrice)}</Typography>
        <Typography sx={{ fontSize: ".8rem", color: "#64748B", mt: 0.4 }}>
      ...
      <TableCell sx={sellerTableCellSx}>
        <Stack spacing={0.8} alignItems="flex-start">
          <SellerStatusChip label={humanizeSellerValue(order.orderStatus)} tone={statusTone(order.orderStatus)} small />
          {manageReturns ? (
            <Typography sx={{ fontSize: ".76rem", color: "#64748B" }}>Managed in returns</Typography>
          ) : null}
        </Stack>
      </TableCell>
      <TableCell sx={sellerTableCellSx}>

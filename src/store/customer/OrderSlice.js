  ...
  async ({ address, jwt, paymentGateway }, { rejectWithValue }) => {
    try {
      ...
      const razorpayKey = String(
        data.razorpay_key ?? data.razorpayKey ?? import.meta.env.VITE_RAZORPAY_KEY_ID ?? '',
      )
      ...
      return rejectWithValue(
        'Unable to initialize payment. Razorpay order details were not returned.',
      )
    } catch (error) {
      const message =
...
export const verifyRazorpayPayment = createAsyncThunk(
  ...
  async (
    { jwt, paymentOrderId, razorpayPaymentId, razorpayOrderId, razorpaySignature },
    { rejectWithValue },
  ) => {
    try {
...
export const markPaymentFailed = createAsyncThunk(
  ...
  async ({ jwt, paymentOrderId, reason }, { rejectWithValue }) => {
    try {
      const response = await api.post(
        '/api/payments/fail',
      ...
      if (error.response)
        return rejectWithValue(error.response.data?.message || 'Failed to mark payment failed')
...
export const selectPaymentConfirmed = (state) => state.orders.paymentConfirmed

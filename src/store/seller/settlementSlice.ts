...
const buildQueryParams = (query?: SettlementQuery) => {
...
export const fetchSettlements = createAsyncThunk<
  ...
  { jwt: string; query?: SettlementQuery },
  { rejectValue: string }
>("settlements/fetchSettlements", async ({ jwt, query }, { rejectWithValue }) => {
  try {
    ...
    if (error.response) return rejectWithValue(error.response.data?.message || "Failed to load settlements");
...
export const fetchSettlementById = createAsyncThunk<
  ...
  { rejectValue: string }
>("settlements/fetchSettlementById", async ({ jwt, id }, { rejectWithValue }) => {
  try {
    ...
    if (error.response) return rejectWithValue(error.response.data?.message || "Failed to load settlement");
...
export const createSettlement = createAsyncThunk<
  Settlement,
  { jwt: string; payload: SettlementPayload },
  { rejectValue: string }
>("settlements/createSettlement", async ({ jwt, payload }, { rejectWithValue }) => {
  try {
    ...
    if (error.response) return rejectWithValue(error.response.data?.message || "Failed to create settlement");
...
export const updateSettlementStatus = createAsyncThunk<
  Settlement,
  { jwt: string; id: number; status: SettlementStatus; remarks?: string },
  { rejectValue: string }
>(
  ...
  async ({ jwt, id, status, remarks }, { rejectWithValue }) => {
    try {
      ...
      if (error.response) return rejectWithValue(error.response.data?.message || "Failed to update status");
...
export const fetchSettlementSummary = createAsyncThunk<
  ...
  { jwt: string; query?: SettlementQuery },
  { rejectValue: string }
>("settlements/fetchSettlementSummary", async ({ jwt, query }, { rejectWithValue }) => {
  try {
    ...
    if (error.response) return rejectWithValue(error.response.data?.message || "Failed to load summary");
    return rejectWithValue("Failed to load summary");
  }
});

const normaliseListResponse = (payload: SettlementListResponse) => {
...
const settlementSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSettlementFilters: (state, action: PayloadAction<SettlementQuery>) => {
